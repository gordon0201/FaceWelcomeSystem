...
namespace FaceWelcome.Service.Services.Implementations
{
    public class WelcomeTemplateService : IWelcomeTemplateService
    {
        ...
        public async Task<GetTemplateResponse> GetTemplateByPhoneNumberAndEventIdAsync(string phoneNumber, Guid eventId)
        {
            try
            {
                var person = await _unitOfWork.PersonRepository.GetPersonByPhone(phoneNumber);
                if (person == null)
                {
                    throw new Exception("Person cannot found");
                }
                var guest = await _unitOfWork.GuestRepository.GetGuestByEventIdAndPersonIdAsync(eventId, person.Id);
                if (guest == null)
                {
                    throw new Exception("Guest cannot found");
                }
                GetTemplateResponse templateResponse = await GetTemplateByGuestIdAsync(guest.Id);
